...
// I worked on this challenge with: Susan
...
// Pseudocode
//  Use function to count number of times name appears in index.
//  Use comparison operators and count method to get output of which name appears most frequently.
//  Declare winner to whichever name has highest count
...
for(var position in voteCount){
  ...
  for(var candidate in voteCount[position]){
    if (voteCount[position][candidate] > count || count == 1){
      officers[position] = candidate; 
      count = voteCount[position][candidate];
    }
  }
}
...
//      That we can use recursive functions when we encounter them.
// 2. Were you able to find useful methods to help you with this?
//      Yes, but didn't implent them that much
